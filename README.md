...
번개장터 운영을 로컬 Mac에서 Codex 또는 Claude와 함께 처리하기 위한 AI 툴킷입니다.
...
`--tool cli`는 `npm install` 뒤 `npm run bunjang -- auth.status`로 래퍼 준비 상태를 확인합니다.
...
- macOS Intel/Apple Silicon, Codex, Claude Code만 고려합니다.
